...
class SplitLib {
  constructor (config) {
    ...
    if (config && config.dustServerAbc) {
      this.dustServerAbc = config.dustServerAbc
    } else {
      this.dustServerAbc = DUST_SERVER_ABC
    }

    if (config && config.dustServerBchn) {
      this.dustServerBchn = config.dustServerBchn
    } else {
      this.dustServerBchn = DUST_SERVER_BCHN
    }
  }
  ...
  async getDustAbc (abcSweeper) {
    try {
      ...
      const opt = {
        method: 'get',
        baseURL: `${this.dustServerAbc}/coins/${addr}`,
      ...
      console.error('Error in getDustAbc()')
      throw err
    }
  }
  ...
  async getDustBchn (abcSweeper) {
    try {
      ...
      const opt = {
        method: 'get',
        baseURL: `${this.dustServerBchn}/coins/${addr}`,
        timeout: 10000 // Exit after 10 seconds if server does not respond.
      }
      const tokenRes = await this.axios.request(opt)
      ...
      console.error('Error in getDustBchn()')
      throw err
    }
  }
  ...
  // txid as input.
  async verifyDustAbc (abcSweeper, txid) {
    try {
      let txidFound = false
      ...
      console.error('Error in verifyDustAbc()')
      throw err
    }
  }

  // Same as the ABC function, but for BCHN.
  async verifyDustBchn (bchnSweeper, txid) {
    try {
      let txidFound = false

      // Refresh the UTXOs
      await bchnSweeper.populateObjectFromNetwork()
      bchnSweeper.paper.balance = bchnSweeper.BCHBalanceFromPaperWallet
      bchnSweeper.paper.utxos = bchnSweeper.UTXOsFromPaperWallet
      ...
      // Loop through each UTXO.
      for (let i = 0; i < bchnSweeper.paper.utxos.bchUTXOs.length; i++) {
        const thisUtxo = bchnSweeper.paper.utxos.bchUTXOs[i]
      ...
      console.error('Error in verifyDustBchn()')
      throw err
    }
  }
